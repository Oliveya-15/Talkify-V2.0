...
export default function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [summary, docs] = await Promise.all([api.analyticsSummary(), api.listDocuments()]);
        setStats(summary);
  ...
  return (
    <AppShell>
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <p className="text-slate-500 mt-1">Here's what's happening with your documents.</p>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
          <StatCard label="Documents" value={stats?.total_documents} loading={loading} />
          <StatCard label="Processed" value={stats?.completed_documents} loading={loading} />
          <StatCard label="Conversations" value={stats?.total_conversations} loading={loading} />
          <StatCard label="Messages sent" value={stats?.total_messages} loading={loading} />
        </div>
...
function StatCard({ label, value, loading }) {
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      ...
      <p className="text-2xl font-bold text-ink mt-1">{loading ? '—' : value ?? 0}</p>
    </div>
  );
}
